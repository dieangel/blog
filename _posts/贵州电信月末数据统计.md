...
```sql
select count(1) from V$C_IMSI  
  where opendate >= to_date('20161201','yyyymmdd') 
		and opendate < to_date('20170101','yyyymmdd'); 
```
**固网**
```sql
...
**C网**
```sql
select count(distinct imsi) from C_IMSI_HIS a 
  where a.otype=3 
    and a.itime >= to_date('20161201','yyyymmdd') 
    and a.itime < to_date('20170101','yyyymmdd');
```
**固网**
```sql
select servicetype,count(*) from v$b_broadband_user 
  where status=2 
    and closedate >= to_date('20161201','yyyymmdd') 
    and closedate < to_date('20170101','yyyymmdd') 
    group by servicetype;
```
## 活跃用户数
```sql
...
select '27' as d1,servicetype,count(*) as c from b_broadband_det12_27 group by servicetype union all
select '25' as d1,servicetype,count(*) as c from b_dialup_det12 where stoptime > to_date('2016-12-25 00:00:00','YYYY-MM-DD HH24:MI:SS') 
                                                                 and stoptime <= to_date('2016-12-26 00:00:00','YYYY-MM-DD HH24:MI:SS') group by servicetype union all
select '26' as d1,servicetype,count(*) as c from b_dialup_det12 where stoptime > to_date('2016-12-26 00:00:00','YYYY-MM-DD HH24:MI:SS') 
                                                                 and stoptime <= to_date('2016-12-27 00:00:00','YYYY-MM-DD HH24:MI:SS') group by servicetype union all
select '27' as d1,servicetype,count(*) as c from b_dialup_det12 where stoptime > to_date('2016-12-27 00:00:00','YYYY-MM-DD HH24:MI:SS') 
                                                                 and stoptime <= to_date('2016-12-28 00:00:00','YYYY-MM-DD HH24:MI:SS') group by servicetype) order by d1,servicetype
```
**C网**
```sql
select day,num from (
	select '25' as day,count(*) as num from c_det12_25 union all
	select '26' as day,count(*) as num from c_det12_26 union all
	select '27' as day,count(*) as num from c_det12_27);
```
# 表空间及各表使用情况
```sql
